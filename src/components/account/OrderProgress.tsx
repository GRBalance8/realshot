...
export const OrderProgress: FC<OrderProgressProps> = ({ status, orderId }) => {
  ...
  return (
    <div className="bg-white rounded-[32px] shadow-lg p-8">
      <h3 className="text-xl font-medium text-blue-900 mb-6">Order Status #{orderId.slice(-6)}</h3>
        ...
        <div className="space-y-6">
          {steps.map((step, index) => (
            <div key={index} className="flex items-center">
              <div className={`relative z-10 w-11 h-11 rounded-full flex items-center justify-center border-2 
                ${step.completed 
              ...
              <div className="ml-4">
                <p className={`font-medium ${step.completed ? 'text-blue-900' : 'text-gray-500'}`}>
                  {step.label}
                </p>
              </div>
            </div>
